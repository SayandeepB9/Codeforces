...
void solve()
{
    ...
    ll diff = 0, rev_diff = 0;
    fr(i,0,n)
    {
        if(s[i]!=t[i])  diff++;
        if(s[i]!=t[n-1-i])  rev_diff++;
    }

    if(diff == 0)   cout<<0<<'\n';
    else if(rev_diff == 0)  cout<<2<<'\n';
    else if(diff > rev_diff)
    {
        if(rev_diff & 1)    cout<<2*rev_diff<<'\n';
        else    cout<<2*rev_diff-1<<'\n';
    }
    else if(diff < rev_diff)
    {
        if(diff & 1)    cout<<2*diff-1<<'\n';
        else    cout<<2*diff<<'\n';
    }
    else
        cout<<2*diff-1<<'\n';
