...
void solve()
{
    ...
    int dp[b.size()+10][c.size()+10];

    fr(i,0,b.size())
    {
        if(i) dp[i][i]=dp[i-1][i-1]+abs(b[i]-c[i]);    else dp[i][i]=abs(b[0]-c[0]);
        fr(j,i+1,c.size())
            if(i) dp[i][j]=min(dp[i-1][j-1]+abs(b[i]-c[j]),dp[i][j-1]);
            else dp[i][j]=min(abs(b[i]-c[j]),dp[i][j-1]);
    }

    cout<<dp[b.size()-1][c.size()-1]<<'\n';
}
