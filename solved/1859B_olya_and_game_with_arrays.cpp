...
void solve()
{
    ...
    fr(i,0,n)
    {
        ll min=a[i][0],s_min=inf;
        fr(j,1,m[i])
        {
            if(a[i][j]<=min)    s_min=min,min=a[i][j];
            else if(a[i][j]<s_min)  s_min=a[i][j];
        }
        mi[i][0]=min,mi[i][1]=s_min;
    }

    ll min=inf,min1=inf;
    fr(i,0,n)
    {
        if(mi[i][1]<min) min=mi[i][1];
        if(mi[i][0]<min1) min1=mi[i][0];
    }
