...
void solve()
{
    ...
    string s;
    cin>>s;
    unordered_map<int,unordered_map<int,si>> a;
    ...
    fr(i,0,q)
    {
        int x,y,l,r;
        cin>>x>>y>>l>>r;
        if(!a[x][y].empty() && (*a[x][y].begin()<l || *--a[x][y].end()>=r))
        {
            cout<<"YES\n";
            continue;
        }
        x=b[r].first-(x-b[l-1].first);
        y=b[r].second-(y-b[l-1].second);
        auto z=a[x][y].lower_bound(l);
        if(z!=a[x][y].end() && *z<r)
        {
            cout<<"YES\n";
            continue;
