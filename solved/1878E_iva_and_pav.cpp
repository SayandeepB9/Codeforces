...
void solve()
{
    ...
    fr(i,0,q)
    {
        ...
        while((1<<m)>k)
        {
            auto lo=a[m].lower_bound(l-1);
            if(lo!=a[m].end()) ma=max(ma,*lo);
            else ma=n;
            m--;
        }
        int mb=n,sk=0;
        while(m>=0)
        {
            auto it=a[m].lower_bound(l-1);
            if(it!=a[m].end() && ((k>>m)&1))    mb=max(min(mb,*it),sk),sk=min(sk,mb);
            else if(!((k>>m)&1))
            {
                if(it!=a[m].end()) sk=min(mb,max(sk,*it));
                else sk=mb;
            }
            m--;
        }
        if(l-1!=ma) cout<<max(ma,mb)<<' ';
        else if(l-1!=mb)  cout<<mb<<' ';
        else cout<<-1<<' ';
        // cout<<ma<<' '<<mb<<'\n';
    }
    cout<<'\n';
