...
void solve()
{
    ...
    cin>>s[0];
    if(s[0]==1);
    else    s[1]=s[0],s[0]=1,k=2;
    fr(i,k,m+k-1)
        cin>>s[i];

    int l=m+k-1,count[l+1]={0},sel=0;
    fr(i,1,l)   count[i]=ceili((s[i]-s[i-1]),d);

    count[l]=(n-s[l-1])/d+1;
    fr(i,2,l)
        if(ceili((s[i]-s[i-2]),d)<count[i]+count[i-1])  sel++;

    if(count[l-1]+count[l]>(n-s[l-2])/d+1)    sel++;
    int sum=0;
    fr(i,0,l+1)   sum+=count[i];
    cout<<sum-(sel!=0)<<' '<<(sel?sel:m)<<'\n';
}
