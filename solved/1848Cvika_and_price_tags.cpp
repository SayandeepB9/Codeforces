...
ll eucleid(int  a, int b)
{
    if(a==0)    return 0;
    if(b==0)    return 1;
    if(a==b)    return 2;
    if(a<b) return 1+eucleid(b,b-a);

    int d=a/b;
    int r=a%b;
    if(d&1) return d+d/2+eucleid(b,r);
    else    return d+d/2+eucleid(r,b);
}
...
void solve()
{
    ...
    fr(i,0,n)   if(a[i] || b[i])    c.pb(eucleid(a[i],b[i]));

    fr(i,1,c.size())
        if(abs(c[i]-c[i-1])%3)  {yn(0); return;}
    yn(1);
}
