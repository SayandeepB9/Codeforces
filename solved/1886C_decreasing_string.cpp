...
void solve()
{
    ...
    ll pos,len=s.size();
    cin>>pos;
    ll k=sqrt(len*(len+1)-2*pos);
    if(len*(len+1)-2*pos>=k*(k+1)) k++;
    k=len-k;
    ...
    fr(i,0,len)
    {
        ...
        {
            if(a[a.size()-1]<=s[i]) a.pb(s[i]);
            else
            {
                while(a.size() && *--a.end()>s[i])
                {
                    b.pb(*--a.end());
                    a.erase(--a.end());
                    if(b.size()>=k) break;
    ...
    while(a.size()+b.size()<len)    a.pb(s[a.size()+b.size()]);
    ...
    k=len-k;
    // cout<<pos<<' '<<k<<'\n';
    pos=pos-(len*(len+1)/2-(k)*(k+1)/2);
    //if(pos>a.size())   cout<<s[k+pos-1];
    cout<<a[pos-1];
    // cout<<pos<<'\n'<<'\n';
}
