...
void solve()
{
    ...
    // memset(b,0,sizeof(b));
    in(a,n-1);
    rev(i,n-2,1)    a[i]=a[i]-a[i-1];

    fr(i,0,n-1)
        if(a[i]>n || b[a[i]])   x.pb(a[i]);
        else    b[a[i]]++;
        
    if(x.size()!=1 && x.size()!=0) yn(0);
    else
    {
        fr(i,1,n+1) if(!b[i])   x.pb(i);
        if(x.size()==3 && x[1]+x[2]==x[0])  yn(1);
        else if(x.size()==1 && x[0]<n+1 && !b[x[0]])  yn(1);
        else    yn(0);
    }
}
