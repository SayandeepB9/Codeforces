...
int minimum(int dp[], int a[], int n, int i)
{
    if(dp[i]!=-1) return dp[i];
    if(i==n-1)  return dp[i]=1;
    if(i+a[i]>n-1)   return (dp[i]=1+minimum(dp,a,n,i+1));
    if(i+a[i]==n-1)  return dp[i]=0;
    return (dp[i]=min(1+minimum(dp,a,n,i+1),minimum(dp,a,n,i+a[i]+1)));
}
